...
struct ContentView: View {
    ...
    var body: some View {
        ...
        .onChange(of: defaultStartDateString) { _ in

            if let date = Self.dateFormatter.date(
                from: defaultStartDateString
            ) {
                startDate = date
            }
        }
    ...
    private static func drawCenteredText(
        ...
        let drawRect = CGRect(
            x: rect.origin.x,
            y: rect.origin.y
                + (rect.height - boundingRect.height) / 2.0,
            width: rect.width,
            height: rect.height
        )
